...
def build():
    ...
    for path in sorted((ROOT/'reports').rglob('paired_results.csv')):
        with path.open(encoding='utf-8-sig',newline='') as f:
            references=sorted({row['run_id'] for row in csv.DictReader(f) if row.get('run_id')})
        ...
        phases.append(dict(phase=path.parent.relative_to(ROOT/'reports').as_posix(),purpose=role,
                           referenced_runs=len(references),raw_runs_included=len(included),missing_run_ids=missing))
