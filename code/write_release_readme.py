"""Write the front page from a named, completed phase of the actual incumbent."""
import argparse,json
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]

def write(phase,variant):
    current=json.loads((ROOT/'INCUMBENT.json').read_text())
    freeze=json.loads((ROOT/current['freeze']).read_text())
    ...
    selected=[row for row in rows if row['task']['variant']==variant]
    if not selected:raise ValueError('No selected runs')
    for row in selected:
        manifest=json.loads((ROOT/'runs'/row['run_id']/'manifest.json').read_text())
        if manifest['source_hashes']!=freeze['source_hashes']:raise ValueError('Readme would describe a different source version')
...
这里是自建本地场景成绩，分布是假设；完整配对、失败与分项时间见 `reports/{phase}/`。本轮没有运行官方演练或正式测试。第三问 G 和原目录 D_COMPACT 保持独立。
...
- `{current['configuration']}`、`{current['freeze']}`：冻结配置与全部源码哈希。
...
'''
    ...
    return dict(version=current['version'],phase=phase,variant=variant,scenarios=summary['n'])

if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('phase');p.add_argument('variant');a=p.parse_args();print(write(a.phase,a.variant))
