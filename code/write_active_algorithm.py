...
def write():
    ...
    base=base.replace('若当前站已经落在某频道的保证清除区内，立即原地清除。',
                      '对本轮刚完成共享测向的频道，若当前站落在其更新后的保证清除区内，则原地清除；进入共享测向循环前已能保证清除的其他频道仍由全局任务调度处理。')
    ...
    if options.get('stable_quadrature'):
        appendix+=['','## 当前启用：与多边形顶点表示无关的区域取样','',
                   '评分专用副本先规范化凸包，沿较长包围盒轴计算分段线性截面宽度，精确积分梯形面积，再在等面积分位上选点，并用固定的交替截面分位选第二坐标。相同几何区域插入共线点、循环移动顶点或反向列举，评分取样在浮点容差内一致。这一性质不包含任意旋转；候选几何规范化N未启用，也不声称整个策略对所有顶点表示都不变。它只稳定有限假设的位置；保守区域、真实观测和退出证明均不因此改变。','']
    ...
    if options.get('compact_optical'):
        ...
        else:appendix+=['当前配置在每轮选定RF候选之后、实际发送RF之前比较这套光学覆盖，RF候选可以是原双侧探测，也可以是M选择的单点探测。原双侧探测的第一端反馈后额外比较光学覆盖的开关未启用；M实际单点反馈后回到全局重规划。','']
