...
from e_policy_presets import FROZEN_E_OPTIONS, FROZEN_E_VERSION
...
def run_case(seed=1, strategy="G", scenario="uniform", reception="mixed",
             error_mode="fixed_hash", count=None, depth=2, beam_width=8,
             opportunistic=True, compute_oracle=True, output=None, policy_options=None,update_latest=True):
    if strategy not in ("A", "B", "C", "D", "E", "F", "G"):
    ...
    if strategy == 'D' and policy_options is None:
        policy_options = dict(FROZEN_D_OPTIONS)
    if strategy == 'E' and policy_options is None:
        policy_options = dict(FROZEN_E_OPTIONS)
    if strategy == 'F' and policy_options is None:
        policy_options = dict(FROZEN_F_OPTIONS)
    if strategy == 'G' and policy_options is None:
    ...
    if strategy in ('E','F','G'):
        allowed_options.update(('replan_measurements','adaptive_rotation','route_centroid',
                                'area_rollout','lateral','replacement_scan','adaptive_reception','shared_limit',
                                'commit_radius','route_hysteresis','eager_count_stop'))
    if strategy in ('F','G'):
        allowed_options.update(('negative_halfplanes','joint_history','task_search','tail_cost',
                                'optical_exclusions','safe_shared','strip_fallback','negative_disks','flexible_search',
                                'forecast_budget','via_clear'))
    if strategy == 'G':
        allowed_options.update(('rotate_search','bend_search','refine_bend','robust_bend'))
    if set(policy_options) - allowed_options or (policy_options and strategy not in ('D','E','F','G')):
        raise ValueError('Invalid policy options')
    if strategy in ('D','E','F','G'):
        defaults = {name: parameter.default for name, parameter in inspect.signature(OptimizedController).parameters.items()
                    if name != 'client'}
        if strategy in ('E','F','G'):
            defaults.update({name: parameter.default for name, parameter in inspect.signature(ReplanningController).parameters.items()
                             if name not in ('client','options')})
        if strategy in ('F','G'):
            defaults.update({name: parameter.default for name, parameter in inspect.signature(StructuralController).parameters.items()
                             if name not in ('client','options')})
        policy_options = {**defaults, **policy_options}
    config = dict(seed=seed, count=count, scenario=scenario, reception=reception, error_mode=error_mode)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ")
    run_id = f"{stamp}-s{seed}-{strategy}-{uuid.uuid4().hex[:8]}"
    ...
    metadata = {"run_id": run_id, "mode": "training", "data_origin": "self_written_simulator",
                "seed": seed, "strategy_version": ('local-D-'+FROZEN_D_VERSION if strategy=='D' else
                                                     'local-E-'+FROZEN_E_VERSION if strategy=='E' else
                                                     'local-F-'+FROZEN_F_VERSION if strategy=='F' else
                                                     'local-G-'+FROZEN_G_VERSION if strategy=='G' else f"local-{strategy}-v1"),
    ...
    controller = (RouteSearchController(client, **policy_options) if strategy == 'G' else
                  StructuralController(client, **policy_options) if strategy == 'F' else
                  ReplanningController(client, **policy_options) if strategy == 'E' else
                  OptimizedController(client, **policy_options) if strategy == 'D' else
    ...
    metrics['average_localization_clear_time_s_per_source']=(metrics['total_time']/metrics['clear_count']
                                                           if metrics['clear_count'] else None)
    if compute_oracle:
    ...
    regions=observation_regions(evaluation['events'])
    if strategy=='F':
        event_times={e['response']['virtual_time_s']:e['index'] for e in evaluation['events']}
        for line in (directory/'requests.jsonl').read_text(encoding='utf-8').splitlines():
            record=json.loads(line)
            if record.get('reason') in ('F_feedback_region','F_optical_negative_region'):
                regions.append({k:record[k] for k in ('channel','polygon','pieces','center','radius')}|
                               {'event_index':event_times[record['virtual_time_s']]})
        regions.sort(key=lambda row:row['event_index'])
...
def main():
    ...
    p.add_argument("--strategy", choices=["A", "B", "C", "D", "E", "F", "G"], default="G")
