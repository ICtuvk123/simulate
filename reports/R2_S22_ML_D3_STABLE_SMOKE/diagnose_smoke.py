...
assert len(rows)==20==len(reg['tasks'])
...
diff=[p['per_source_saved_s'] for p in paired];rng=random.Random(819346)
...
result=dict(all_postchecks_valid=all(all_valid),all_source_hashes_match=same_hash,contractions=stats,
    mean_per_source_saved_s=statistics.mean(diff),paired_smoke_bootstrap_95_interval_s=[boot[500],boot[19499]],
