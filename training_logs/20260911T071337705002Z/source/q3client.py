...
class HttpTransport:
    def __init__(self, base_url="http://127.0.0.1:2026"):
        ...
        if (parsed.scheme != "http" or parsed.hostname not in ("127.0.0.1", "::1", "localhost")
            ...
            raise ProtocolError("Simulator address must be an HTTP loopback origin")
        self.host, self.port = parsed.hostname, parsed.port or 80

    def __call__(self, path, body, timeout):
        # Direct connection: no environment proxy and no automatic redirects.
...
class Client:
    def __init__(self, robot_id, journal, base_url="http://127.0.0.1:2026",
