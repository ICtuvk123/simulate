...
def run_case(seed=1, strategy="C", scenario="uniform", reception="mixed",
    ...
    if set(policy_options) - allowed_options or (policy_options and strategy not in ('D','E')):
    ...
    metadata = {"run_id": run_id, "mode": "training", "data_origin": "self_written_simulator",
                "seed": seed, "strategy_version": ('local-D-'+FROZEN_D_VERSION if strategy=='D' else
                                                     'local-E-'+FROZEN_E_VERSION if strategy=='E' else f"local-{strategy}-v1"),
    ...
    controller = (ReplanningController(client, **policy_options) if strategy == 'E' else
...
def main():
    ...
    p.add_argument("--strategy", choices=["A", "B", "C", "D", "E"], default="C")
