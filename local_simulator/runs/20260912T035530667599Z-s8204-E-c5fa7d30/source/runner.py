...
def run_case(seed=1, strategy="E", scenario="uniform", reception="mixed",
    ...
    if strategy not in ("A", "B", "C", "D", "E", "F"):
    ...
    if set(policy_options) - allowed_options or (policy_options and strategy not in ('D','E','F')):
        raise ValueError('Invalid policy options')
    if strategy in ('D','E','F'):
    ...
    metadata = {"run_id": run_id, "mode": "training", "data_origin": "self_written_simulator",
                "seed": seed, "strategy_version": ('local-D-'+FROZEN_D_VERSION if strategy=='D' else
                                                     ...
                                                     'local-F-'+FROZEN_F_VERSION if strategy=='F' else f"local-{strategy}-v1"),
    ...
    controller = (StructuralController(client, **policy_options) if strategy == 'F' else
...
def main():
    ...
    p.add_argument("--strategy", choices=["A", "B", "C", "D", "E", "F"], default="E")
