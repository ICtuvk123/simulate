...
class MaximumSourcesKnown(Exception):
    """Internal control flow after a real accepted detection proves the upper bound."""
...
class ReplanningController(OptimizedController):
    def __init__(self,client,replan_measurements=True,adaptive_rotation=False,
                 ...
                 commit_radius=0.,route_hysteresis=0.,eager_count_stop=False,**options):
        ...
        self.eager_count_stop=bool(eager_count_stop)
        self.rotation_selected=False
        self.refinements={}
        self.optical_attempts={}

    def measure(self,point,channel):
        count_before=len(set(self.polygons)|self.cleared)
        response=super().measure(point,channel)
        if self.eager_count_stop and count_before<16 and len(set(self.polygons)|self.cleared)==16:
            self.search_complete=True
            self.record(reason='E_public_maximum_discovered_stop_unknown_scan',point=point)
    ...
    def scan_stop(self,point,force=False):
        ...
        try:
            ...
            # The triggering real feedback was already journaled and applied.
            # No synthetic no-signal replies or future coverage are manufactured.
            pass
