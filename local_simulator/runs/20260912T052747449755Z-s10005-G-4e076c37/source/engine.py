"""Independent Q3 simulator. Public physics, locally chosen scene distributions.
...
def generate_sources(seed, count=None, scenario="uniform", reception="mixed"):
    rng = random.Random(f"q3-world-v1:{seed}")
    n = rng.randint(10, 16) if count is None else count
    if type(n) is not int or not 10 <= n <= 16:
        raise ValueError("Q3 requires 10 to 16 sources")
    ...
    for ch in channels:
        ...
        sources.append(Source(ch, x, y, radius))
...
class Session:
    ...
    def __init__(self, seed=0, count=None, scenario="uniform", reception="mixed",
                 error_mode="fixed_hash", robot_id="local-robot", sources=None,
                 monotonic=time.monotonic, timestamp=time.time):
        self._sources = sources if sources is not None else generate_sources(seed, count, scenario, reception)
        ...
        if any(not 1 <= s.channel <= 20 or math.hypot(s.x, s.y) > 1800 + 1e-8
               or not 1000 <= s.radius <= 1500 for s in self._sources):
            raise ValueError("invalid Q3 source")
        if error_mode not in ("fixed_hash", "smooth", "plus_one", "minus_one"):
            raise ValueError("unknown error model")
        self._error_seed = hashlib.sha256(f"q3-error-v1:{seed}".encode()).digest()
    ...
    def evaluation(self):
        ...
        return {"jammer_count": len(self._sources), "clear_count": len(self.cleared),
