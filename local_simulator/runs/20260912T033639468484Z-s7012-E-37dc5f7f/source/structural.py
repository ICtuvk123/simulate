...
class StructuralController(ReplanningController):
    def __init__(self,client,negative_halfplanes=True,joint_history=True,
                 ...
                 safe_shared=False,strip_fallback=True,negative_disks=False,flexible_search=False,**options):
    ...
    def refine_and_clear(self,ch):
        if not self.joint_history and not self.tail_cost and not self.optical_exclusions and not self.negative_disks:
    ...
    def run(self):
        ...
        for decision in range(220):
            ...
            if kind=='scan':
                ...
                self.refine_and_clear(index);self.scan_stop(self.client.ledger.position,force=self.flexible_search)
