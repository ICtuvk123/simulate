...
class RouteSearchController(StructuralController):
    def __init__(self, client, rotate_search=False, bend_search=False, refine_bend=False,
                 robust_bend=False, **options):
        super().__init__(client, **options)
        self.rotate_search = bool(rotate_search)
        self.bend_search = bool(bend_search)
        self.refine_bend = bool(refine_bend)
        self.robust_bend = bool(robust_bend)
    ...
    def rebuild_budgeted_forecast(self):
        ...
        for original in selected:
            ...
            def task_region(task_index):
                return ([points[task_index]] if task_index<len(self.remaining_sites)
                        else self.polygons[pending[task_index-len(self.remaining_sites)]])
            before_region=[current] if j==0 else task_region(order[j-1])
            after_region=task_region(order[j+1]) if j+1<len(order) else []
            ...
            if self.refine_bend:
                # Each required residual area induces a disk constraint on the
                # new station. Its feasible intersection is convex. We keep only
                # explicitly certified iterates; conservative rejection may
                # shorten the interval but cannot create a coverage claim.
                def safe(q):
                    return all(self.proves(self.negatives[ch]+others+anchors+[q]) for ch in unknown)
                for goal in goals:
                    if math.dist(goal,original)<1:
                        continue
                    def at(t):
                        return tuple(original[k]+t*(goal[k]-original[k]) for k in (0,1))
                    def cost(t):
                        q=at(t)
                        return math.dist(before,q)+(math.dist(q,after) if j+1<len(order) else 0.)
                    lo,hi=0.,1.
                    for _ in range(18):
                    ...
                    t=(lo+hi)/2
                    if cost(t)>=cost(0)-1e-6:
                        continue
                    if not safe(at(t)):
                        lo,hi=0.,t
                        for _ in range(10):
                            mid=(lo+hi)/2
                            if safe(at(mid)):lo=mid
                            else:hi=mid
                        t=lo
                    if t>1e-5 and safe(at(t)):
                        candidates.append(at(t))
            old_length = route_length(current,points,order)
            best = (old_length,original)
            for q in candidates:
                if self.robust_bend:
                    # Squared-distance difference is affine in the neighboring
                    # target position: vertices certify its entire convex region.
                    # Demand no longer entry OR exit for every feasible neighbor,
                    # not merely a shorter tour through estimated centers.
                    if any(sum((q[k]-v[k])**2-(original[k]-v[k])**2 for k in (0,1))>1e-6
                           for v in before_region+after_region):
                        continue
