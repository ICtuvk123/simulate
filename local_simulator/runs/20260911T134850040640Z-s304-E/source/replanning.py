...
def area_rollout_station(poly,current,previous,lateral=1.0):
    ...
    best=None
    for q in candidates:
        if any(math.dist(q,p)<.05 for p in previous) or max(math.dist(q,v) for v in poly)>1000-1e-4:
            continue
...
class ReplanningController(OptimizedController):
    def __init__(self,client,replan_measurements=True,adaptive_rotation=False,
                 ...
                 replacement_scan=False,**options):
    ...
    def refine_and_clear(self,ch):
        if not self.replan_measurements and not self.area_rollout:
        ...
        for _ in range(iterations):
            ...
            choice=(area_rollout_station(poly,current,previous,self.lateral) if self.area_rollout
